...
if (!$isEmpresa && !$isEgresado) { if (!headers_sent()) header('Location: /View/LoginEgresados.php?redirect=1'); else echo '<script>location.href="/View/LoginEgresados.php?redirect=1"</script>'; exit; }
...
<body>
    ...
    <aside class="sidebar sidebar--drawer">
      ...
      <div class="menu">
        <?php if ($tipo==='empresa'): ?>
          ...
          <a href="/View/MisVacantes.php">Mis Vacantes</a>
          <a href="/View/Chat.php" class="active">Chat</a>
          <a href="/Auth/logout.php">Salir</a>
        <?php else: ?>
          <a href="/View/BuscarVacantes.php">Buscar Vacantes</a>
          <a href="/View/Vacantes.php">Ver Vacantes</a>
          <a href="/View/PerfilEgresado.php">Mi Perfil</a>
          <a href="/View/Chat.php" class="active">Chat</a>
  ...
  <script>
    ...
    async function loadConversations(){
      const res = await fetch('/Api/chat_api.php?action=list_conversations');
    ...
    async function loadMessages(){
      ...
      const res = await fetch('/Api/chat_api.php?action=fetch_messages&conversation_id='+currentConversation);
      const data = await res.json();
    ...
    async function searchParticipants(q){
      const res = await fetch('/Api/chat_api.php?action=search_participants&q='+encodeURIComponent(q||''));
      const data = await res.json();
