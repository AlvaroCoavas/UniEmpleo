...
if (!$conexion) {
    header("Location: /View/PublicarVacante.php?error=1&detail=" . urlencode('Error de conexión'));
    exit;
}
...
if ($titulo === '' || $descripcion === '' || $ubicacion === '' || $tipo_empleo === '') {
    header("Location: /View/PublicarVacante.php?missing=1");
    exit;
}

$stmt = mysqli_prepare($conexion, "INSERT INTO vacantes (empresa_id, titulo, descripcion, ubicacion, tipo_empleo) VALUES (?, ?, ?, ?, ?)");
if (!$stmt) {
    header("Location: /View/PublicarVacante.php?error=1&detail=" . urlencode('Error preparando consulta'));
    exit;
}
mysqli_stmt_bind_param($stmt, 'issss', $empresa_id, $titulo, $descripcion, $ubicacion, $tipo_empleo);

if (mysqli_stmt_execute($stmt)) {
    header("Location: /View/PublicarVacante.php?ok=1");
    exit;
} else {
    $err = htmlspecialchars(mysqli_error($conexion));
    header("Location: /View/PublicarVacante.php?error=1&detail=" . urlencode($err));
    exit;
}
