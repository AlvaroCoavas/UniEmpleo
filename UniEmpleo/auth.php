...
function require_egresado(): void {
    ...
    if (session_is_timed_out()) {
        secure_logout('LoginEgresados.php?expired=1');
    }

    if (!isset($_SESSION['correo'])) {
        if (!headers_sent()) {
            header('Location: LoginEgresados.php?redirect=1');
        } else {
            echo '<script>location.href="LoginEgresados.php?redirect=1";</script>';
        }
        exit;
    }
...
function require_empresa(): void {
    ...
    if (session_is_timed_out()) {
        secure_logout('LoginEmpresa.html?expired=1');
    }
    ...
    if (!isset($_SESSION['empresa_id'])) {
        if (!headers_sent()) {
            ...
            echo '<script>location.href="LoginEmpresa.html?redirect=1";</script>';
        }
        exit;
    }
